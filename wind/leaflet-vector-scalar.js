...
L.Control.VelocityNEW = L.Control.extend({
  options: {
    position: "bottomleft",
    emptyString: "",
  ...
  onAdd: function onAdd(map) {
    this._container = L.DomUtil.create("div", "leaflet-control-velocitynew");
    L.DomEvent.disableClickPropagation(this._container); // map.on("mousemove", this._onMouseMove, this);

    this._container.innerHTML = this.options.emptyString;
    if (this.options.leafletVelocityNEW.options.onAdd) this.options.leafletVelocityNEW.options.onAdd();
    return this._container;
  },
  onRemove: function onRemove(map) {
    // map.off("mousemove", this._onMouseMove, this);
    if (this.options.leafletVelocityNEW.options.onRemove) this.options.leafletVelocityNEW.options.onRemove();
  },
  vectorToSpeed: function vectorToSpeed(uMs, vMs, unit) {
    var velocitynewAbs = Math.sqrt(Math.pow(uMs, 2) + Math.pow(vMs, 2)); // Default is m/s

    if (unit === "k/h") {
      return this.meterSec2kilometerHour(velocitynewAbs);
    } else if (unit === "kt") {
      return this.meterSec2Knots(velocitynewAbs);
    } else {
      return velocitynewAbs;
    }
  },
  vectorToDegrees: function vectorToDegrees(uMs, vMs, angleConvention) {
    ...
    var velocitynewAbs = Math.sqrt(Math.pow(uMs, 2) + Math.pow(vMs, 2));
    var velocitynewDir = Math.atan2(uMs / velocitynewAbs, vMs / velocitynewAbs);
    var velocitynewDirToDegrees = velocitynewDir * 180 / Math.PI + 180;

    if (angleConvention === "bearingCW" || angleConvention === "meteoCCW") {
      velocitynewDirToDegrees += 180;
      if (velocitynewDirToDegrees >= 360) velocitynewDirToDegrees -= 360;
    }

    return velocitynewDirToDegrees;
  },
  ...
  _onMouseMove: function _onMouseMove(e) {
    ...
    var pos = this.options.leafletVelocityNEW._map.containerPointToLatLng(L.point(e.containerPoint.x, e.containerPoint.y));

    var gridValue = this.options.leafletVelocityNEW._windy.interpolatePoint(pos.lng, pos.lat);

    var htmlOut = "";

    if (gridValue && !isNaN(gridValue[0]) && !isNaN(gridValue[1]) && gridValue[2]) {
      htmlOut = "<strong>" + this.options.velocitynewType + " Direction: </strong>" + self.vectorToDegrees(gridValue[0], gridValue[1], this.options.angleConvention).toFixed(2) + "°" + ", <strong>" + this.options.velocitynewType + " Speed: </strong>" + self.vectorToSpeed(gridValue[0], gridValue[1], this.options.speedUnit).toFixed(2) + this.options.speedUnit;
    } else {
...
L.control.velocitynew = function (options) {
  return new L.Control.VelocityNEW(options);
};
...
L.ScalarLayer = (L.Layer ? L.Layer : L.Class).extend({
  ...
  _onMouseMove: function (e) {
    ...
    var gridValue = 0;
    // console.log(self._scalar)
    if (self._scalar) {
      gridValue = self._scalar.interpolatePoint(
        pos.lng,
        pos.lat
    ...
    if (self.options.displayValues) {
      var htmlOut = gridValue ? gridValue.toFixed(2) : 0;
      let dom = document.querySelector("#showRainValue")
      if(dom){
        dom.innerHTML = "降水量： "+htmlOut+' 毫米'

        let offsetX = dom.offsetWidth/2
        dom.style.position = "absolute";
        // dom.style.width = "auto";
        dom.style['white-space']='nowrap';
        dom.style['pointer-events'] = "none";
        dom.style.left = e.containerPoint.x- offsetX+'px';
        dom.style.top = e.containerPoint.y-40+'px';
      }
    }
  ...
  onDrawLayer: function onDrawLayer(overlay, params) {
    ...
    this._timer = setTimeout(function () {
      self._startScalar();
    }, 0); // showing velocitynew is delayed
  },
  ...
  _initMouseHandler: function _initMouseHandler(voidPrevious) {
    ...
    if (!this._mouseControl && this.options.displayValues) {
      ...
      options["leafletVelocityNEW"] = this;
      this._mouseControl = L.control.velocitynew(options).addTo(this._map);
    }
  },
  // 拖拽时 隐藏 windy canvas
...
L.VelocityNEWLayer = (L.Layer ? L.Layer : L.Class).extend({
  options: {
    displayValues: true,
    displayOptions: {
      velocitynewType: "VelocityNEW",
      position: "bottomleft",
      emptyString: "No velocitynew data"
    },
    maxVelocityNEW: 10,
  ...
  _initWindy: function _initWindy(self) {
    ...
    this._canvasLayer._canvas.classList.add("velocitynew-overlay");
  ...
  _hideWind: function _hideWind() {
    var canvas = document.querySelector('.velocitynew-overlay');
...
L.velocitynewLayer = function (options) {
  return new L.VelocityNEWLayer(options);
};
/**
...
var µ = function () {
  ...
  function colorInterpolator(start, end) {
    ...
    return function (i, a) {
      // console.log("aaa",i,a)
      return [Math.floor(r + i * Δr), Math.floor(g + i * Δg), Math.floor(b + i * Δb), final || a];
    };
  }
  /**
...
var Scalar = function Scalar(params) {
  ...
  var MIN_VELOCITYNEW_INTENSITY = params.minValue || 193; // velocitynew at which particle intensity is minimum (m/s)

  var MAX_VELOCITYNEW_INTENSITY = params.maxValue || 328; // velocitynew at which particle intensity is maximum (m/s)
  ...
  var SCALE = {
    bounds: [MIN_VELOCITYNEW_INTENSITY, MAX_VELOCITYNEW_INTENSITY],
    gradient: {} // 计算色值及step
  ...
  var setColorScale = function setColorScale() {
    if (params.colorScale) {
      return params.colorScale
    }
    let n = parseFloat(((MAX_VELOCITYNEW_INTENSITY - MIN_VELOCITYNEW_INTENSITY - 0.01) / (COLORSCALE.length - 1)).toFixed(4)),
      m = MIN_VELOCITYNEW_INTENSITY;
  ...
  var createScalarBuilder = function createScalarBuilder(res) {
    ...
    if (res.length > 1) {
      ...
      _data = _data1.data.map(function (v, i) {
        ...
        return Math.sqrt(Math.pow(v, 2) + Math.pow(_data2.data[i], 2))
      });
    } else {
      _data1 = res[0];
      _data = _data1.data.map(function (v, i) {
...
var Windy = function Windy(params) {
  var MIN_VELOCITYNEW_INTENSITY = params.minVelocityNEW || 0; // velocitynew at which particle intensity is minimum (m/s)

  var MAX_VELOCITYNEW_INTENSITY = params.maxVelocityNEW || 10; // velocitynew at which particle intensity is maximum (m/s)

  var VELOCITYNEW_SCALE = (params.velocitynewScale || 0.005) * (Math.pow(window.devicePixelRatio, 1 / 3) || 1); // scale for wind velocitynew (completely arbitrary--this value looks nice)
  ...
  var setOptions = function setOptions(options) {
    if (options.hasOwnProperty("minVelocityNEW")) MIN_VELOCITYNEW_INTENSITY = options.minVelocityNEW;
    if (options.hasOwnProperty("maxVelocityNEW")) MAX_VELOCITYNEW_INTENSITY = options.maxVelocityNEW;
    if (options.hasOwnProperty("velocitynewScale")) VELOCITYNEW_SCALE = (options.velocitynewScale || 0.005) * (Math.pow(window.devicePixelRatio, 1 / 3) || 1);
  ...
  var interpolateField = function interpolateField(grid, bounds, extent, callback) {
    ...
    var velocitynewScale = VELOCITYNEW_SCALE * Math.pow(mapArea, 0.4);
    var columns = [];
    var x = bounds.x;
      ...
      for (var y = bounds.y; y <= bounds.yMax; y += 2) {
        ...
        if (coord) {
          ...
          if (isFinite(λ)) {
            ...
            if (wind) {
              wind = distort(projection, λ, φ, x, y, velocitynewScale, wind);
  ...
  var animate = function animate(bounds, field) {
    function windIntensityColorScale(min, max) {
      colorScale.indexFor = function (m) {
        // map velocitynew speed to a style
    ...
    var colorStyles = windIntensityColorScale(MIN_VELOCITYNEW_INTENSITY, MAX_VELOCITYNEW_INTENSITY);
