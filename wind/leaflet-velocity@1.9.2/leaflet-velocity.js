...
L.Control.Velocity = L.Control.extend({
  ...
  onAdd: function onAdd(map) {
    ...
    this._container.style="background:#FFF;font-size:15px;font-weight:bold"
  ...
  getWindLevel: function getWindLevel(windSpeed) {
    const num = Number(windSpeed)
    if (num < 1) {
      return 0
    } else if (num < 5) {
      return 1
  ...
  /**
   * @function checkWindDirect
   * @description 自定义的角度转换
   */
  checkWindDirect(deg) {
    var cardinalDirection = ""
    if (deg >= 0 && deg < 11.25 || deg >= 348.75) {
      cardinalDirection = '南风';
    } else if (deg >= 11.25 && deg < 33.75) {
      cardinalDirection = '东南风偏南';
    } else if (deg >= 11.25 && deg < 56.25) {
      cardinalDirection = '东南风';
    } else if (deg >= 56.25 && deg < 78.75) {
      cardinalDirection = '东南风偏东';
    } else if (deg >= 78.25 && deg < 101.25) {
      cardinalDirection = '东风';
    } else if (deg >= 101.25 && deg < 123.75) {
      cardinalDirection = '东北风偏东';
    } else if (deg >= 123.75 && deg < 146.25) {
      cardinalDirection = '东北风';
    } else if (deg >= 146.25 && deg < 168.75) {
      cardinalDirection = '东北风偏北';
    } else if (deg >= 168.75 && deg < 191.25) {
      cardinalDirection = '北风';
    } else if (deg >= 191.25 && deg < 213.75) {
      cardinalDirection = '西北风偏北';
    } else if (deg >= 213.75 && deg < 236.25) {
      cardinalDirection = '西北风';
    } else if (deg >= 236.25 && deg < 258.75) {
      cardinalDirection = '西北风偏西';
    } else if (deg >= 258.75 && deg < 281.25) {
      cardinalDirection = '西风';
    } else if (deg >= 281.25 && deg < 303.75) {
      cardinalDirection = '西南风偏西';
    } else if (deg >= 303.75 && deg < 326.25) {
      cardinalDirection = '西南风';
    } else if (deg >= 326.25 && deg < 348.75) {
      cardinalDirection = '西南风偏南';
    }
  ...
  _onMouseMove: function _onMouseMove(e) {
    ...
    if (gridValue && !isNaN(gridValue[0]) && !isNaN(gridValue[1]) && gridValue[2]) {
      ...
      var cardinal = this.options.showCardinal ? `(${self.checkWindDirect(deg)})`:''
      htmlOut = "<strong> ".concat(this.options.velocityType, " ")
        .concat(this.options.directionString, ": </strong> ") // “方向”的标签描述
        .concat(deg.toFixed(2), "\xB0") // 角度
        .concat(cardinal, ", <strong> ")
        .concat(this.options.velocityType, " ")
        .concat(this.options.speedString, ": </strong> ")
        .concat(self.vectorToSpeed(gridValue[0], gridValue[1], this.options.speedUnit)
          .toFixed(2), " ").concat(this.options.speedUnit)
          .concat( "(" + this.getwindlevel(gridValue[2] * 3.6) + "级)");
    } else {
      htmlOut = this.options.emptyString;
    }

    self._container.innerHTML = htmlOut;
    self._container.style.background = "rgba(0,0,0,0.6)";
    self._container.style.color = "#FFF";
    let offsetX = self._container.offsetWidth/2
    self._container.parentNode.style.position = "absolute";
    // self._container.parentNode.style.width = "400px";

    self._container.parentNode.style['white-space']='nowrap';
    self._container.parentNode.style['pointer-events'] = "none";
    self._container.parentNode.style.left = e.containerPoint.x- offsetX+'px';
    self._container.parentNode.style.top = e.containerPoint.y-40+'px'
...
L.VelocityLayer = (L.Layer ? L.Layer : L.Class).extend({
  ...
  onDrawLayer: function onDrawLayer(overlay, params) {
    ...
    this._timer = setTimeout(function () {
      self._startWindy();
    }, 0); // showing velocity is delayed 原本是750
  },
  ...
  _initWindy: function _initWindy(self) {
    ...
    // this._map.on("dragstart", self._clearWind);
